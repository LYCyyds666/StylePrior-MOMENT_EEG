...
变体：
    full        : 完整模型，结果与 two_stage_training_multidataset.py APAVA 完全一致
    wo_sisl     : w/o SISL  gamma=1, beta=0，风格变换退化为恒等变换
    wo_hse      : w/o HSE   去掉 SharedHyperExpert 的贡献
    wo_stsa     : w/o STSA  去掉测试时自适应

用法：
    python ablation_apava.py --variant full
    python ablation_apava.py --variant wo_sisl
    python ablation_apava.py --variant wo_hse
    python ablation_apava.py --variant wo_stsa
"""
...
def train_one_fold(train_loader, val_loader):
    ...
    for epoch in range(epochs):
        ...
        for bd in train_loader:
            ...
            if num_classes == 2:
                tr_pr.extend(torch.softmax(logits, 1)[:, 1].detach().cpu().numpy())
            else:
        ...
        with torch.no_grad():
            for bd in val_loader:
                ...
                if num_classes == 2:
                    vl_pr.extend(torch.softmax(logits, 1)[:, 1].cpu().numpy())
                else:
...
def evaluate_baseline(model, loader):
    ...
    with torch.no_grad():
        for bd in loader:
            ...
            l.extend(lbl.numpy())
            if num_classes == 2:
                pr.extend(torch.softmax(logits, 1)[:, 1].cpu().numpy())
            else:
...
def run_stsa(model, test_loader, tta_lr=5e-4, tta_batch_size=64):
    ...
    pa = np.array(all_pr) if num_classes == 2 else np.concatenate(all_pr, 0)
...
def k_fold_cross_validation():
    ...
    print(f"\n{'='*55}")
    print(f"[{variant}] APAVA Results (K={k_folds})")
    print(f"{'='*55}")
    ...
    sp = f"./results/ablation_apava_{variant}_seed{seed}.pkl"
